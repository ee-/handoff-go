...
`go update` 是显式维护命令，不是 workflow state，也不会被 contributor 内容或 watch tick 触发。它将最新的可信 `ee-/handoff-go` 解析为一个 immutable commit，刷新本 repository 的 project-local skill、managed bootstrap pin 以及已启用的 watch 副本，并打开一个可 review 的 governance proposal（参见 `skills/handoff-go/references/adoption.md`）。
